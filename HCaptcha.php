<?php
class HCaptcha extends CInputWidget
{

    const JS_API_URL  = 'https://hcaptcha.com/1/api.js';
    ...
    public function init()
    {
        parent::init();
        if (empty($this->key)) {
            if (!empty(Yii::app()->hCaptcha->key)) {
                $this->key = Yii::app()->hCaptcha->key;
            } else {
    ...
    protected function customFieldPrepare()
    {
        if ($this->hasModel()) {
            ...
            $inputName = $this->name;
            $inputId   = 'hcaptcha-' . $this->name;
        }

        if (empty($this->jsCallback)) {
            $jsCode = "var hcaptchaCallback = function(response){jQuery('#{$inputId}').val(response);};";
        } else {
            $jsCode = "var hcaptchaCallback = function(response){jQuery('#{$inputId}').val(response); {$this->jsCallback}(response);};";
        }
        $this->jsCallback = 'hcaptchaCallback';
        $cs               = Yii::app()->getClientScript();
        $cs->registerScript('hcaptcha-js', $jsCode, CClientScript::POS_BEGIN);
     ...
     * @return string hcaptcha-compatible base64 encoded encrypted binary data.
     */
